...
function set-remote-plan-args() {
    ...
    if [[ -n "$INPUT_VAR_FILE" ]]; then
        for file in $(echo "$INPUT_VAR_FILE" | tr ',' '\n'); do
            cp "$file" "$INPUT_PATH/zzzz-pops-rox-tf-gh-actions-$AUTO_TFVARS_COUNTER.auto.tfvars"
    ...
    if [[ -n "$INPUT_VARIABLES" ]]; then
        echo "$INPUT_VARIABLES" >"$STEP_TMP_DIR/variables.tfvars"
        cp "$STEP_TMP_DIR/variables.tfvars" "$INPUT_PATH/zzzz-pops-rox-tf-gh-actions-$AUTO_TFVARS_COUNTER.auto.tfvars"
    fi
...
STEP_TMP_DIR="/tmp"
JOB_TMP_DIR="$HOME/.pops-rox-tf-gh-actions"
WORKSPACE_TMP_DIR=".pops-rox-tf-gh-actions/$(random_string)"
...
function fix_owners() {
    ...
    if [[ -d "$GITHUB_WORKSPACE/.pops-rox-tf-gh-actions" ]]; then
        chown -R --reference "$GITHUB_WORKSPACE" "$GITHUB_WORKSPACE/.pops-rox-tf-gh-actions" || true
        debug_cmd ls -la "$GITHUB_WORKSPACE/.pops-rox-tf-gh-actions"
    fi

    debug_cmd ls -la "$HOME"
    if [[ -d "$HOME/.pops-rox-tf-gh-actions" ]]; then
        chown -R --reference "$HOME" "$HOME/.pops-rox-tf-gh-actions" || true
        debug_cmd ls -la "$HOME/.pops-rox-tf-gh-actions"
    fi
    ...
    if [[ -d "$INPUT_PATH" ]]; then
        debug_cmd find "$INPUT_PATH" -regex '.*/zzzz-pops-rox-tf-gh-actions-[0-9]+\.auto\.tfvars' -print -delete || true
    fi
}
